...
if (isset($_POST['btn-cadastrar'])) {
    ...
    $query = "INSERT INTO produtos (pro_nome, pro_quantidade, pro_preco, pro_imagem, pro_fornecedor) VALUES ('$nome', '$qtd', '$preco', '$imagem', '$fornecedor')";
...
if (isset($_POST['btn-editar'])) {
    ...
    $sql = "UPDATE produtos SET pro_nome='$nome', pro_quantidade='$qtd_nova', pro_preco='$preco', pro_imagem='$imagem', pro_fornecedor='$fornecedor' WHERE pro_id=$id";
    if (mysqli_query($conexao, $sql) && $dif != 0) {
        mysqli_query($conexao, "INSERT INTO historico (his_produto, his_quantidade, his_fornecedor) VALUES ('$nome', '$dif', '$fornecedor')");
    }
}
...
<head>
    ...
    <style>
        ...
        select { padding: 8px; margin: 5px 0; width: 100%; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px; }    
...
<div class="container">
    <div class="header">
        ...
        <a href="/auth/logout.php" class="btn btn-del">Sair da Conta</a>
        <a href="/index.php" class="btn btn-cancel">Visitar a loja</a>
    </div>
    <div class="form-section">
        <?php if ($editar_dados): ?>
            <h2>Editar Produto: <?php echo $editar_dados['pro_nome']; ?></h2>
            <form method="POST" action="gestao-produtos.php">
                ...
                <label>Imagem:</label>
                <input type="text" name="imagem" value="<?php echo $editar_dados['pro_imagem']; ?>" required>
                <label>Fornecedor/Motivo</label>
                <select name="fornecedor2" required>
                    <option value="<?php echo $editar_dados['pro_fornecedor2']; ?>">seu fornecedor é <?php echo $editar_dados['pro_fornecedor2']; ?></option>
                    <?php
                    $fornecedores = mysqli_query($conexao, "SELECT * FROM fornecedor ORDER BY for_social ASC");
                    while ($forn = mysqli_fetch_assoc($fornecedores)):
                    ?>
                        <option value="<?php echo $forn['for_social']; ?>" <?php echo ($forn['for_social'] == $editar_dados['pro_fornecedor']) ? 'selected' : ''; ?>>
                            <?php echo $forn['for_social']; ?>
                        </option>
                    <?php endwhile; ?>
                </select>
        ...
        <?php else: ?>
            ...
            <form method="POST" action="gestao-produtos.php">
                ...
                <select name="fornecedor" required>
                    ...
                    while ($forn = mysqli_fetch_assoc($fornecedores)):
                    ?>
                        <option value="<?php echo $forn['for_social']; ?>"><?php echo $forn['for_social']; ?></option>
                    <?php endwhile; ?>
                </select>
    ...
    <form method="GET" class="search-bar">
        ...
        <?php if (!empty($_GET['busca'])): ?>
            <a href="gestao-produtos.php" class="btn btn-cancel">Limpar</a>
        <?php endif; ?>
    </form>
